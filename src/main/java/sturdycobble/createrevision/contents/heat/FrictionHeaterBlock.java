...
public class FrictionHeaterBlock extends DirectionalKineticBlock implements ITE<FrictionHeaterTileEntity> {
    ...
    private static final VoxelShaper FRICTION_HEATER_SHAPER = VoxelShaper.forDirectional(
            Block.box(0.0D, 0.1D, 0.0D, 16.0D, 14.0D, 16.0D), Direction.UP);
    ...
    public BlockState getStateForPlacement(BlockItemUseContext context) {
        ...
        return defaultBlockState().setValue(FACING, context.getPlayer() !=
                null && context.getPlayer().isSecondaryUseActive() ? preferredFacing : preferredFacing.getOpposite());
    }
    ...
    public VoxelShape getShape(BlockState state, IBlockReader world, BlockPos pos, ISelectionContext context) {
        return FRICTION_HEATER_SHAPER.get(state.getValue(FACING));
    }
